...
window.onerror = (msg, url, line) => {
  ...
  if (display) display.textContent = `Fehler: ${msg} (Zeile ${line})`;
};
...
closeBtn.addEventListener('click', () => {
  if (invoke) invoke('close_popup').catch(() => {});
});
...
// Read data from URL query params
...
if (dataStr) {
  try {
    ...
    if (!selectedModel) {
      translationDisplay.textContent = 'Fehler: Kein Modell ausgewählt (Einstellungen → Modell wählen)';
    } else {
...
async function translate(text, ollamaUrl, selectedModel, systemPrompt, sourceLang, targetLang) {
  ...
  let fullText = '';
  try {
    console.log(`Fetching ${ollamaUrl}/api/generate with model ${selectedModel}`);
    if (invoke && listen) {
      const [fnChunk] = await Promise.all([
